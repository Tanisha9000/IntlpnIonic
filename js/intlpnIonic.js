...
var intlpnCtrl = function( $ionicModal, $scope, intlpnUtils ) {
    ...
    self._updateDialCode = function(newDialCode) {
     ...
     //   console.log(self.countryDialCode)
    };
    ...
    return {
     ...
     //       console.log('ngModelCtrl')
      //      console.log(ngModelCtrl)
       ...
       //             console.log(scope.countryDialCode)
                }
                if( modelValue )
...
//                console.log(scope.phone+scope.number)
//                console.log(scope.phone)
                var full_number = scope.phone+scope.number
                 ngModelCtrl.$setViewValue( full_number );
...
//                ngModelCtrl.$setViewValue( scope.phone );
       ...
       //                 console.log(scope.countryDialCode)
        ...
        //        console.log('simer6')
                ...
                if( scope.national ) {
                    var phone = intlTelInputUtils.formatNumberByType(scope.phone, scope.number, scope.isocode, intlTelInputUtils.numberFormat.INTERNATIONAL);
                    var dial = scope.intlpnHelper.getDialCode(phone);
                    return dial === scope.dialCode && scope.isValid( modelValue, scope.isocode );
                } else {
          //          console.log('valid form'+ modelValue)
         ...
         //   console.log(input.length)
           // console.log(typeof(input))
            Object.keys(input).forEach(function(key,value) {
               // console.log(input)
                console.log(key)
                var val = input[key];
            //    console.log(val)
               // console.log(input[key].attr('id'))
                //console.log(typeof(val))
                //console.log(value)
               // var result = document.getElementById("input1")
               if(key<2){
                var wrappedResult = angular.element(val);
               // console.log(wrappedResult)
               // get element by id: input1
               
                wrappedResult.bind('focus', function() {
                //console.log($)
                console.log('bind start')
                if( scope.national ) {
           //         console.log('national')
                } else {
          //          console.log('else non national')
           //         console.log(scope.phone)
                    if( !scope.phone ) {
                        scope.$apply(function() {
              //              console.log('dialcode'+scope.dialCode)
                            scope.phone = scope.dialCode;
                            scope.countryDialCode = scope.dialCode;
                            input[key].setSelectionRange( scope.dialCode.length,scope.dialCode.length );
                        });
                    }
                }
            })
...
//                        console.log('focus out else if')
//                        console.log(scope.dialCode)
//                        console.log(scope.phone)
//                         console.log(scope.number)
//                        console.log(scope.intlpnHelper.getDialCode(scope.phone))
                        scope.$apply(function() {
                            scope.full_number = scope.phone+scope.number
                           // scope.phone = scope.phone;
                        });
                //        console.log(scope.full_number)
                    }
                }
            });
        }
        ...
        template: '<div class="item item-input intlpn-container">' +
                        ...
                        '<input id="input1" intlpn-formatter national-mode="nationalMode" iso-code="{{isocode}}" type="tel" placeholder="{{placeholder||\'test\'}}" ng-model="phone" class="myinput">' +
                        '<input id="input2" type="text" ng-model="number" class="myinput1" placeholder="(123)456-789">' +
//                        '<input id="input3" type="hidden" ng-model="full_number">' +
                '</div>'
    };
}])
